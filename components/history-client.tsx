...
export function HistoryClient({ generations }: { generations: Generation[] }) {
    ...
    <div className="relative min-h-screen bg-[#080808] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-5 py-8 md:px-8 md:py-10">
        <header className="mb-6">
          <p className="mb-1 font-mono text-xs uppercase tracking-[0.25em] text-white/35">Your archive</p>
          <h1 className="text-2xl font-semibold tracking-tight text-white">History</h1>
          <p className="mt-1 text-sm text-white/45">Every idea you turned into content, with its best score.</p>
        </header>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((g, i) => {
                  ...
                  return (
                    <tr key={g.id} className={cn("transition-colors hover:bg-white/[0.025]", !isLast && "border-b border-white/5")}>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="inline-flex items-center gap-3">
                          <button type="button" onClick={() => setOpenId(g.id)} className="text-sm font-medium text-violet-400 hover:text-violet-300 transition-colors">
                            View
                          </button>
                          {confirmDeleteId === g.id ? (
                            <div className="inline-flex items-center gap-1.5">
                              <button type="button" onClick={() => handleDelete(g.id, g.createdAt)} className="text-xs font-medium text-rose-400 hover:text-rose-300 transition-colors">
                                Confirm
                              </button>
                              <button type="button" onClick={() => setConfirmDeleteId(null)} className="text-white/30 hover:text-white/60 transition-colors">
                                <X className="h-3.5 w-3.5" />
                              </button>
                            </div>
                          ) : (
                            <button type="button" onClick={() => handleDelete(g.id, g.createdAt)} className="text-white/25 hover:text-rose-400 transition-colors" aria-label="Delete entry">
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 backdrop-blur-sm sm:items-center" onClick={() => setOpenId(null)}>
            <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/15 bg-[#0e0e0e] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="mb-4 flex items-start justify-between gap-4">
                ...
                <button type="button" onClick={() => setOpenId(null)} className="rounded-lg p-1.5 text-white/30 transition-colors hover:bg-white/10 hover:text-white">
                  <X className="h-4 w-4" />
                </button>
              </div>
